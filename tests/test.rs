...
impl<T> Op<T> {
    fn execute_b(self, heap: &mut BinaryHeap<T>) -> Option<T> where T: Clone + Ord {
        match self {
            Peek => heap.peek().cloned(),
    ...
    fn execute_p(self, heap: &mut BinomialHeap<T>) -> Option<T> where T: Clone + Ord {
        match self {
            Peek => heap.peek().cloned(),
            Push(item) => { heap.push(item); None }
...
impl<T: Arbitrary> Arbitrary for Op<T> {
    fn arbitrary<G: Gen>(gen: &mut G) -> Self {
        match gen.gen_range(0, 10) {
            0 => Peek,
            1 => PushPop(T::arbitrary(gen)),
            2 => Pop,
            3 => Replace(T::arbitrary(gen)),
    ...
    fn shrink(&self) -> Box<Iterator<Item = Self>> {
        match *self {
            ...
            Peek | Pop => Box::new(std::iter::empty()),
...
fn test_drain() {
    fn t(ops: Vec<Op<i32>>) -> bool {
        ...
        let mut p_vec: Vec<_> = p.drain().collect();
        p_vec.sort();
...
fn test_into_iter() {
    fn t(ops: Vec<Op<i32>>) -> bool {
        ...
        let mut p_vec: Vec<_> = p.into_iter().collect();
        p_vec.sort();
